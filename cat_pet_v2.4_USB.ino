 ...
 */
 ...
 // This helps verify that the XIAO + SD + TinyUSB are stable as a pure USB drive.
 ...
 bool sdInitWithSpeed() {
  // Try only conservative, lower SPI speeds since this SD
  // setup is a bit touchy at higher frequencies.
 ...
 float getUsedPercent() {
  // Assumes caller only invokes this when usbActive == false so we
  // aren't racing with an active MSC transfer.
   selectSD();
 ...
 void setup() {
   ...
   Serial.print("Board: ");
   Serial.println("XIAO Seeed RP2040");
   Serial.println();
   ...
   // In full cat mode, load all frames and measure initial usage
 #if !USB_MSC_ONLY
   if (!loadAllFrames()) {
 ...
 #else
   // MSC-ONLY MODE: do NOT touch SD or draw cat after this.
   Serial.println("====================================");
   Serial.println("USB MSC-ONLY MODE");
   Serial.println("No cat animation, MCU will stay idle");
   Serial.println("Safe to format/use SD from the PC");
   Serial.println("====================================");
   Serial.println();
 #endif
   ...
   // Step 1: Configure MSC class and start it (not ready yet)
   Serial.println("Step 1: Configuring USB MSC (not ready)...");
   ...
   Serial.println("  Marked NOT ready");
 
   bool usbOk = usb_msc.begin();
   Serial.print("  begin() returned: ");
   Serial.println(usbOk ? "true" : "false");
 
   // If CDC was already enumerated, force re-enumeration so host sees MSC too
   if (TinyUSBDevice.mounted()) {
     Serial.println("  Device already mounted, forcing re-enumeration");
   ...
   if (!usbOk) {
     ...
     // Step 2: SD info + capacity, then mark unit ready
     Serial.println("Step 2: Getting SD card info...");
     uint32_t sectorCount = sd.card()->sectorCount();
     Serial.print("  Sector count: ");
     Serial.println(sectorCount);
 
     Serial.println("Step 3: Finalizing USB MSC (capacity + ready)...");
     ...
     Serial.println();
     Serial.println("====================================");
     Serial.println("UNPLUG AND REPLUG USB NOW IF NEEDED");
     Serial.println("Wait 10 seconds after replugging");
     Serial.println("'CatPet SD' disk should appear");
     Serial.println("====================================");
     Serial.println();
 
     // In full cat mode the TFT will already be showing animation.
     // In MSC-only mode we can show a simple status text.
 #if USB_MSC_ONLY
     selectTFT();
 ...
 #if USB_MSC_ONLY
 void loop() {
   // In MSC-only mode we let TinyUSB handle everything in the background.
   // Just idle here so we never touch SD/TFT from the MCU side.
   delay(1000);
 }
