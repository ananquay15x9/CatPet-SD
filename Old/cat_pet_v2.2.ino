 ...
 * 
 * Version: 2.2 (RAM-optimized) bigger scale
 */
...
static const float SCALE = 2.5f;      // 160 / 64 = 2.5x (fills width, crops height)
...
void drawFrameToSprite(uint16_t* frameBuffer) {
  ...
  // Push sprite to screen (will be cropped top/bottom by 4 pixels each)
  selectTFT();
...
void setup() {
  ...
  // Note: Sprite is 160px tall but screen is 128px, so top/bottom will crop 16px each
  spr.setColorDepth(16);
  spr.createSprite(SPRITE_W, SPRITE_H);
  spr.fillSprite(BG_COLOR);

  // Calculate centered position for 160x160 sprite on 160x128 screen
  // X: (160-160)/2 = 0px from left (fills width)
  // Y: (128-160)/2 = -16px (crops 16px top, 16px bottom)
